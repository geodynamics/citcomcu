      ...
      for(i=1;i<=vpts;i++)   {
	    ...
	    Vzy[i] += (VY[j]* E->gNX[e].vpt[GNVXINDEX(2,j,i)] + VZ[j]* E->gNX[e].vpt[GNVXINDEX(1,j,i)]);
          ...
          for(j=1;j<=ends;j++)  {
            Vzz[i] +=  VZ[j]* E->gNX[e].vpt[GNVXINDEX(2,j,i)];
            Vxx[i] +=  ( VX[j]* E->gNX[e].vpt[GNVXINDEX(0,j,i)]
                        +VZ[j]*E->N.vpt[GNVINDEX(j,i)] )*rtf[3][i];
            Vxz[i] +=  VX[j]*E->gNX[e].vpt[GNVXINDEX(2,j,i)] + rtf[3][i]*(VZ[j]
              *E->gNX[e].vpt[GNVXINDEX(0,j,i)]-VX[j]*E->N.vpt[GNVINDEX(j,i)]);
            Vyy[i] += ( (VY[j]*E->gNX[e].vpt[GNVXINDEX(1,j,i)]
              + VX[j]*E->N.vpt[GNVINDEX(j,i)]*cos(rtf[1][i]))/sin(rtf[1][i])
              + VZ[j]*E->N.vpt[GNVINDEX(j,i)] )*rtf[3][i];
            Vxy[i] += ( (VX[j]*E->gNX[e].vpt[GNVXINDEX(1,j,i)]
              - VY[j]*E->N.vpt[GNVINDEX(j,i)]*cos(rtf[1][i]))/sin(rtf[1][i])
              + VY[j]*E->gNX[e].vpt[GNVXINDEX(0,j,i)])*rtf[3][i];
            Vzy[i] += VY[j]*E->gNX[e].vpt[GNVXINDEX(2,j,i)] + rtf[3][i]*(VZ[j]
              *E->gNX[e].vpt[GNVXINDEX(1,j,i)]/sin(rtf[1][i])-VY[j]*E->N.vpt[GNVINDEX(j,i)]); 
