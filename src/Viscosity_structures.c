     ...
     struct All_variables *E;
{
  ...
  if (E->viscosity.RHEOL==1)    {
    for(l=1;l<=E->viscosity.num_mat;l++) {
     ...
     if (E->parallel.me==0)fprintf(stderr,"this option for rheology is not supported\n");
       ...
       return_horiz_ave(E,E->T,E->Have.T);
            ...
            for(l=1;l<=E->viscosity.num_mat;l++) {
              fprintf(E->fp,"\tlayer %d/%d: E=%g T1=%g N0=%g Z0=%g\n",
                      ...
                      E->viscosity.E[l-1],E->viscosity.T[l-1],E->viscosity.N0[l-1],E->viscosity.Z[l-1]);
            }
       ...
       if (E->control.adi_heating)  {
	    ...
	    e = (i-1)%E->lmesh.elz+1;
            ...
            for(jj=1;jj<=vpts;jj++) {
               ...
               EEta[(i-1)*vpts + jj] 
		       = tempa*exp(E->viscosity.E[l-1]*(temp2-(temp-temp1))/temp2)
                       *(E->data.visc_factor+slope*(zz-zbotm));
               }
            }
        }
     ...
     int propogate;
{
    ...
    two = 2.0;
      ...
      for(e=1;e<=nel;e++)   {
        ...
        for(jj=1;jj<=vpts;jj++)
              EEta[(e-1)*vpts + jj] = two*EEta[(e-1)*vpts+jj]/(one+scale*pow(EEta[(e-1)*vpts+jj
],exponent1) );
            }
	...
	for(j=1;j<=ppts;j++)  {
	  ...
	  for(i=1;i<=ends;i++)   {
	    ...
	    Vxyz[3][j]+= VV[3][i]*E->gNX[e].ppt[GNPXINDEX(2,i,j)];
            ...
            Vxyz[5][j]+=VV[1][i]*E->gNX[e].ppt[GNPXINDEX(2,i,j)]
                      + rtf[3][j]*(VV[3][i]*E->gNX[e].ppt[GNPXINDEX(0,i,j)]
                      -VV[1][i]*E->N.ppt[GNPINDEX(i,j)]);
            Vxyz[6][j]+=VV[2][i]*E->gNX[e].ppt[GNPXINDEX(2,i,j)]
              + rtf[3][j]*(VV[3][i]*E->gNX[e].ppt[GNPXINDEX(1,i,j)]/sin(rtf[1][j])
