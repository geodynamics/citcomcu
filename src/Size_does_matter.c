...
void get_global_shape_fn(E,el,pressure,rtf,sphere,level)
     struct All_variables *E;
     int el;
     int pressure,level,sphere;
     double rtf[4][9];
{	
  int i,j,k,d,e;
		    ...
		    E->GNX[level][el].vpt[GNVXINDEX(d-1,j,k)] /= jacobian;
              ...
              if (sphere)   {
                 for(d=1;d<=dims;d++)  {
                   x[d] = 0;
                   for(i=1;i<=ends;i++)
                     x[d] += E->XX[level][d][E->IEN[level][el].node[i]]*
                 ...
                 for(d=1;d<=dims;d++)         
                    LGNX[d][j] =
                            bc[d][1]*E->GNX[level][el].vpt[GNVXINDEX(0,j,k)]
                          + bc[d][2]*E->GNX[level][el].vpt[GNVXINDEX(1,j,k)]
                          + bc[d][3]*E->GNX[level][el].vpt[GNVXINDEX(2,j,k)];
                 for(j=1;j<=ends;j++)
                 for(d=1;d<=dims;d++)         
		     ...
		     for(d=1;d<=dims;d++)  {
		        ...
		        for(e=1;e<=dims;e++)
                  ...
                  if (sphere)   {
                    ...
                    for(d=1;d<=dims;d++)         
                      LGNX[d][j] =
                            bc[d][1]*E->GNX[level][el].ppt[GNPXINDEX(0,j,k)]
                          + bc[d][2]*E->GNX[level][el].ppt[GNPXINDEX(1,j,k)]
                          + bc[d][3]*E->GNX[level][el].ppt[GNPXINDEX(2,j,k)];
                    for(j=1;j<=ends;j++)
                    for(d=1;d<=dims;d++)         
...
void get_rtf(E,el,pressure,rtf,lev)
     struct All_variables *E;
     int lev,el,pressure;
     double rtf[4][9];
{
  ...
  if (pressure==0)    {
    for(k=1;k<=vpts;k++) {       /* all of the vpoints */
      ...
      for(d=1;d<=dims;d++)
        for(i=1;i<=ends;i++)
          x[d] += E->XX[lev][d][E->IEN[lev][el].node[i]]*
                E->N.vpt[GNVINDEX(i,k)];
...
void construct_c3x3matrix_el (E,el,cc,ccx,lev,pressure)
     struct All_variables *E;
     struct CC *cc;
     struct CCX *ccx;
     int lev,el,pressure;
{
  ...
  if (pressure==0)           {
    for(k=1;k<=vpts;k++)           {       /* all of the vpoints */
      ...
      for(d=1;d<=dims;d++)
          for(a=1;a<=ends;a++)
            x[d] += E->XX[lev][d][E->IEN[lev][el].node[a]]
...
{ 
  ...
  for(ii=0;ii<=top;ii++)     { /* ii=0 for bottom ii=1 for top */ 
...
void get_global_1d_shape_fn1(E,el,GM,dGammax,top)
     struct All_variables *E;
     int el,top;
     struct Shape_function1 *GM;
     struct Shape_function1_dA *dGammax;
...
{ 
  ...
  double myatan(),temp[9],area,centre[4],rtf[4][9];
  ...
  for (lv=E->mesh.levmax;lv>=E->mesh.levmin;lv--)   {
   ...
   if (E->control.Rsphere)   {
      ...
      dx1 = max( fabs(E->SXX[lv][1][n[3]]-E->SXX[lv][1][n[1]]),
                 fabs(E->SXX[lv][1][n[2]]-E->SXX[lv][1][n[4]]) );
      E->ECO[lv][e].size[1] = dx1*E->ECO[lv][e].centre[3];

      dx2 = max( fabs(E->SXX[lv][2][n[3]]-E->SXX[lv][2][n[1]]),
                 fabs(E->SXX[lv][2][n[2]]-E->SXX[lv][2][n[4]]) );
      E->ECO[lv][e].size[2] = dx2*E->ECO[lv][e].centre[3]
                             *sin(E->ECO[lv][e].centre[1]);

      dx3 = 0.25*(E->SXX[lv][3][n[5]]+E->SXX[lv][3][n[6]]
                 +E->SXX[lv][3][n[7]]+E->SXX[lv][3][n[8]]
                 -E->SXX[lv][3][n[1]]-E->SXX[lv][3][n[2]]
                 -E->SXX[lv][3][n[3]]-E->SXX[lv][3][n[4]]);
      ...
      for(node=1;node<=enodes[E->mesh.nsd];node++)  {
        temp[node] = 0.0;
        for(nint=1;nint<=vpts;nint++)
          temp[node] += E->GDA[lv][e].vpt[nint]*g_point[nint].weight[E->mesh.nsd-1]
