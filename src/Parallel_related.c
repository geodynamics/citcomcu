 ...
 if (been_here ==0 )   {
  for (k=1;k<=E->parallel.no_neighbors;k++)     {
   ...
   for (k=1;k<=E->parallel.no_neighbors;k++)     {
	...
	S[k][1] = E->parallel.me;
        MPI_Isend(S[k],2,MPI_INT,target_proc,
          E->parallel.mst1[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        }           /* for k */

   for (k=1;k<=E->parallel.no_neighbors;k++)     {
        target_proc = E->parallel.neighbors[k];
        idb ++;
        MPI_Irecv(R[k],2,MPI_INT,target_proc,
          E->parallel.mst1[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        }           /* for k */
 ...
 MPI_Request request[100];
   ...
   for (k=1;k<=E->parallel.no_neighbors;k++)     {
	...
	kk = E->parallel.traces_transfer_number[k]*2+1;
        MPI_Isend(E->PINS[k],kk,MPI_INT,target_proc,
          E->parallel.mst1[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        idb ++;
	kk = E->parallel.traces_transfer_number[k]*2*E->mesh.nsd+1;
        MPI_Isend(E->PVV[k],kk,MPI_FLOAT,target_proc,
          E->parallel.mst2[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        idb ++;
        MPI_Isend(E->PXX[k],kk,MPI_DOUBLE,target_proc,
          E->parallel.mst3[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        }         
     }           /* for k */
   ...
   for (k=1;k<=E->parallel.no_neighbors;k++)     {
	...
	kk = E->parallel.traces_receive_number[k]*2+1;
        MPI_Irecv(E->RINS[k],kk,MPI_INT,target_proc,
          E->parallel.mst1[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        idb ++;
	kk = E->parallel.traces_receive_number[k]*2*E->mesh.nsd+1;
        MPI_Irecv(E->RVV[k],kk,MPI_FLOAT,target_proc,
          E->parallel.mst2[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        idb ++;
        MPI_Irecv(E->RXX[k],kk,MPI_DOUBLE,target_proc,
          E->parallel.mst3[E->parallel.me][target_proc],MPI_COMM_WORLD,&request[idb-1]);
        }         
     }           /* for k */
 ...
 for (i=1;i<=E->mesh.nsd;i++)  {
   ...
   for (k=1;k<=2;k++)    
     if (E->parallel.NUM_PASS[levmax].bound[i][k])   {
        ...
        if (target_proc!=E->parallel.me) {
          ...
          target_proc,E->parallel.mst[E->parallel.me][target_proc][k],MPI_COMM_WORLD,&request[idb-1]);
          }
        }           /* for k */
   ...
   for (k=1;k<=2;k++)     
     if (E->parallel.NUM_PASS[levmax].bound[i][k])   {
        ...
        if (target_proc!=E->parallel.me) {
          idb ++;
          MPI_Irecv(R[k],E->parallel.NUM_NEQ[lev].pass[i][k],MPI_DOUBLE,
          target_proc,E->parallel.mst[E->parallel.me][target_proc][k],MPI_COMM_WORLD,&request[idb-1]);
          }
        }           /* for k */
 ...
 for (i=1;i<=E->mesh.nsd;i++)  {
   ...
   for (k=1;k<=2;k++)    
     if (E->parallel.NUM_PASS[levmax].bound[i][k])   {
        ...
        if (target_proc!=E->parallel.me) {
          idb ++;
          MPI_Isend(S[k],E->parallel.NUM_NODE[lev].pass[i][k],MPI_FLOAT,
          target_proc,E->parallel.mst[E->parallel.me][target_proc][k],MPI_COMM_WORLD,&request[idb-1]);
          }
        }           /* for k */
   ...
   for (k=1;k<=2;k++)     
     if (E->parallel.NUM_PASS[levmax].bound[i][k])   {
        ...
        if (target_proc!=E->parallel.me) {
          idb ++;
          MPI_Irecv(R[k],E->parallel.NUM_NODE[lev].pass[i][k],MPI_FLOAT,
          target_proc,E->parallel.mst[E->parallel.me][target_proc][k],MPI_COMM_WORLD,&request[idb-1]);
          }
        }           /* for k */
