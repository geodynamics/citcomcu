...
{ 
	...
	E->lmesh.NEQ[lev] = eqn_count;
        ...
        j1=1; j2=nox-1;
        if (E->parallel.me_loc[1]==E->parallel.nprocx-1) j2 = nox;

        i1=1; i2=noz-1;
	if (E->parallel.me_loc[3]==E->parallel.nprocz-1) i2 = noz;

        k1=1; k2=noy-1;
        if (E->parallel.me_loc[2]==E->parallel.nprocy-1) k2 = noy;
...
{	
  ...
  for(lev=E->mesh.levmax;lev>=E->mesh.levmin;lev--){
	  ...
	  if (E->NODE[lev][i] & TXEDGE) E->TW[lev][i] /= 2.0;
	  if (E->NODE[lev][i] & TZEDGE) E->TW[lev][i] /= 2.0;
	  if (E->NODE[lev][i] & TYEDGE) E->TW[lev][i] = max(1,E->TW[lev][i]/2);
	  }
...
{ 
  ...
  for (el=1; el<=E->lmesh.nel; el++)  {
    ...
    llayer = layers(E,x3);
    if (llayer)  { /* for layers:1-lith,2-upper and 3-lower mantle */
	  E->mat[el] = llayer;
	  }
