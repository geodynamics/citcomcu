...
{ 
  ...
  nox=E->mesh.nox; noz=E->mesh.noz; noy=E->mesh.noy;
...
/* 2nd dimension */
  ...
  for (j=1;j<E->segment.ylayers;j++)  
    dxx[j] = (E->segment.yylayer[j]-E->segment.yylayer[j-1])
            /(E->segment.nylayer[j]-E->segment.nylayer[j-1]);
  j=1;
      ...
      for (k=1;k<=noy;k++)
        for (i=1;i<=nox;i++)
          for (j=1;j<=noz;j++)  {
            ...
            E->XX[lev][1][nodel] = E->SXX[lev][3][nodel]
                   *sin(E->SXX[lev][1][nodel])*cos(E->SXX[lev][2][nodel]);
            E->XX[lev][2][nodel] = E->SXX[lev][3][nodel]
                   *sin(E->SXX[lev][1][nodel])*sin(E->SXX[lev][2][nodel]);
            E->XX[lev][3][nodel] = E->SXX[lev][3][nodel]
                   *cos(E->SXX[lev][1][nodel]);
            }
    }
 else {
