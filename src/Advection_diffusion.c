    ...
    if (E->control.Rsphere)  {
        for(i=1;i<=VPOINTS3D;i++) {
          ...
          for(j=1;j<=ENODES3D;j++) {
            prod1 = (u1 * E->gNX[el].vpt[GNVXINDEX(0,j,i)]*rtf[3][i] +
                     u2 * E->gNX[el].vpt[GNVXINDEX(1,j,i)]*rtf[2][i] +
 ...
 if(diffusion){
   if (E->control.CART3D)   
	...
	Eres[j]=0.0;
        for(i=1;i<=vpts;i++) 
	    Eres[j] -=
		PG.vpt[GNVINDEX(j,i)] * E->gDA[el].vpt[i] * (dT[i] - Q + v1[i]*tx1[i] + v2[i]*tx2[i] + v3[i]*tx3[i]) +
		diff*E->heating_latent[el]*E->gDA[el].vpt[i] * (E->gNX[el].vpt[GNVXINDEX(0,j,i)]*tx1[i] +
	...
	Eres[j]=0.0;
	    ...
	    Eres[j] -=
		PG.vpt[GNVINDEX(j,i)] * E->gDA[el].vpt[i] * (dT[i] - Q + v1[i]*tx1[i] + v2[i]*tx2[i] + v3[i]*tx3[i]) +
		diff*E->heating_latent[el]*E->gDA[el].vpt[i] * 
             (E->gNX[el].vpt[GNVXINDEX(0,j,i)]*tx1[i]*rtf[3][i] +
	      E->gNX[el].vpt[GNVXINDEX(1,j,i)]*tx2[i]*rtf[2][i] +
	      E->gNX[el].vpt[GNVXINDEX(2,j,i)]*tx3[i] ); 
	    }
    }
	...
	for(a=1;a<=ends;a++)
		...
		if (!onedfns++) get_global_1d_shape_fn(E,el,&GM,&dGamma,1);
...
if (E->control.adi_heating)  {
  ...
  if (E->parallel.me==0)  fprintf(E->fp,"%g %g\n",temp5,temp6);
    ...
    for (e=1;e<=E->lmesh.nel;e++)  {
       ...
       for (i=1;i<=ends;i++)    {
          ...
          temp2 = temp2 + temp1*(1.0-E->Fas670[j])*E->Fas670[j]
             *E->V[3][j]*(E->T[j]+E->data.surf_temp)*E->data.disptn_number;
