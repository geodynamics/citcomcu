...
if (E->parallel.me==0) fprintf(stderr,"ok1\n");

    global_default_values(E); 
if (E->parallel.me==0) fprintf(stderr,"ok2\n");
    read_initial_settings(E);  
if (E->parallel.me==0) fprintf(stderr,"ok3\n");
    (E->problem_derived_values)(E);   /* call this before global_derived_  */
    global_derived_values(E);

if (E->parallel.me==0) fprintf(stderr,"ok4\n");
    parallel_domain_decomp1(E);

if (E->parallel.me==0) fprintf(stderr,"ok5\n");
    allocate_common_vars(E);                 

if (E->parallel.me==0) fprintf(stderr,"ok6\n");
    (E->problem_allocate_vars)(E);
    (E->solver_allocate_vars)(E);

if (E->parallel.me==0) fprintf(stderr,"ok6a\n");
    construct_ien(E);
if (E->parallel.me==0) fprintf(stderr,"ok9\n");
    construct_masks(E);		/* order is important here */
if (E->parallel.me==0) fprintf(stderr,"ok10\n");
    construct_id(E);
    construct_lm(E);
if (E->parallel.me==0) fprintf(stderr,"ok11\n");
    construct_sub_element(E);
if (E->parallel.me==0) fprintf(stderr,"ok12\n");

    node_locations (E); 

if (E->parallel.me==0) fprintf(stderr,"ok7a\n");
    construct_mat_group(E);

    (E->problem_boundary_conds)(E);
    check_bc_consistency(E);

if (E->parallel.me==0) fprintf(stderr,"ok13\n");

    parallel_shuffle_ele_and_id(E);

if (E->parallel.me==0) fprintf(stderr,"ok14\n");
    parallel_communication_routs(E);

    construct_shape_functions(E);
if (E->parallel.me==0) fprintf(stderr,"ok15\n");
    mass_matrix(E);

if (E->parallel.me==0) fprintf(stderr,"ok16\n");
    (E->problem_initial_fields)(E);   /* temperature/chemistry/melting etc */

if (E->parallel.me==0) fprintf(stderr,"ok17\n");
    common_initial_fields(E);  /* velocity/pressure/viscosity (viscosity must be done LAST) */
if (E->parallel.me==0) fprintf(stderr,"ok18\n");
     ...
     struct All_variables *E;
{
  ...
  E->parallel.automa=0; E->parallel.nprocx=1; E->parallel.nprocz=1; E->parallel.nprocy=1;
